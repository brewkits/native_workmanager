  ...
  group('Migration Tool CLI', () {
    test('bin/migrate.dart exists and is executable', () {
      ...
      expect(file.existsSync(), isTrue,
          reason: 'bin/migrate.dart must exist for pub to expose it');
    });

    test('Running migration tool with --dry-run executes successfully',
        () async {
      // Use the example directory as the target project
      final result = await Process.run('dart',
          ['run', 'bin/migrate.dart', '--path', 'example', '--dry-run']);

      expect(result.exitCode, equals(0),
          reason: 'Migration tool should exit with code 0 on dry run');
