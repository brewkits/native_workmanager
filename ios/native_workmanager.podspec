...
- Built-in HTTP workers (request, upload, download, sync)
  ...
  # KMP WorkManager Framework (kmpworkmanager v3.5.0)
  # Downloaded from GitHub Releases to stay under the pub.dev 100 MB package limit.
  # RELEASE STEP: attach the rebuilt 3.5.0 KMPWorkManager.xcframework.zip to the v1.8.0
  # release. The git-tracked copy under ios/Frameworks/ is already 3.5.0 for local/CI
  ...
  s.prepare_command = <<-CMD
    ...
    if [ ! -d "Frameworks/KMPWorkManager.xcframework" ]; then
      echo "Downloading KMPWorkManager.xcframework v3.5.0..."
      mkdir -p Frameworks
      curl -L --retry 3 -o /tmp/KMPWorkManager.xcframework.zip \
        "https://github.com/brewkits/native_workmanager/releases/download/v1.8.0/KMPWorkManager.xcframework.zip"
