...
/// A genuinely valid 32x32 8-bit RGB PNG (solid colour, 99 bytes).
///
/// The previous fixture called itself a "minimal valid PNG" but was not one: its
/// IDAT chunk carried a wrong CRC and the bytes after it did not form a valid
/// IEND chunk. `file(1)` still reported "PNG image data, 32 x 32" because it only
/// reads the header, and iOS's decoder was lenient enough to accept it — but
/// Android's BitmapFactory validates the whole stream and correctly refused it,
/// so every ImageProcessWorker test failed on Android with "Failed to decode
/// image". Android was right; the fixture was wrong.
///
/// Generated with correct per-chunk CRCs. If this needs regenerating, the
/// invariant to check is that every chunk's CRC32 covers `type + data` and that
/// the stream ends with a zero-length IEND.
