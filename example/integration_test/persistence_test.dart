...
void main() {
  ...
  group('Persistence & SQLite Integration Tests', () {
    ...
    testWidgets('Verify task status persistence after enqueue', (WidgetTester tester) async {
      ...
      expect(status, isNotNull, reason: 'Status should be persisted in SQLite immediately');
      
      await Future.delayed(const Duration(seconds: 1));
      status = await NativeWorkManager.getTaskStatus(taskId: taskId);
      expect(status, anyOf([TaskStatus.pending, TaskStatus.running]), reason: 'Task should be in a valid persisted state');
    });

    testWidgets('Verify allTasks() retrieves records from SQLite', (WidgetTester tester) async {
      ...
      for (final id in tasks) {
        expect(retrievedIds.contains(id), isTrue, reason: 'Task $id should be found in allTasks()');
      }
    });

    testWidgets('Verify task chain persistence', (WidgetTester tester) async {
      final chainName = 'chain-persist-${DateTime.now().millisecondsSinceEpoch}';
      ...
      expect(chainTasks.isNotEmpty, isTrue, reason: 'Chain steps should be persisted as individual tasks');
    });
  });
}
